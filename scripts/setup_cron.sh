...
install_auto() {
    ...
    # Create new crontab
    {
        ...
        echo "45 9 * * 1-5 cd $PROJECT_DIR && PYTHONPATH=. python3 $SCRIPT_DIR/readiness_check.py --fix --spawn-fixer --quick >> $LOG_DIR/readiness.log 2>&1"
        echo "# Midday check at 12:15 PM — verify sessions ran, data fresh"
        echo "15 12 * * 1-5 cd $PROJECT_DIR && PYTHONPATH=. python3 $SCRIPT_DIR/readiness_check.py --fix --spawn-fixer --quick >> $LOG_DIR/readiness.log 2>&1"
        echo "# Pre-close check at 3:45 PM — last chance to catch issues before EOD"
        echo "45 15 * * 1-5 cd $PROJECT_DIR && PYTHONPATH=. python3 $SCRIPT_DIR/readiness_check.py --fix --spawn-fixer --quick >> $LOG_DIR/readiness.log 2>&1"
    } | crontab -
