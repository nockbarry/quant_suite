...
emit_auto_entries() {
    ...
    echo ""
    echo "$CRON_AUTO_MARKER - Health Check + Missed-Job Catch-up"
    echo "# Every 10 min on trading days: revive tmux/sentinel, run catch-up for any job whose slot was missed while the host was off"
    echo "*/10 5-19 * * 1-5 $SCRIPT_DIR/athena_scheduler.sh health-check >> $LOG_DIR/health_check.log 2>&1"
    echo ""
