...
def add_note(thesis_id: str, note: str) -> dict | None:
    """Add a timestamped note to a thesis. Dual-writes."""
    from datetime import datetime
    ...
    with get_db() as session:
        ...
        timestamped = f"[{datetime.utcnow().strftime('%Y-%m-%d %H:%M')}] {note}"
        notes.append(timestamped)
